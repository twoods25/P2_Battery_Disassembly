...
def show_detection(frame: np.ndarray, result: DetectionResult) -> None:
    """Displays detection result with numbered blobs and an info bar.

    Draws on a copy of the provided frame. Never modifies the original.
    World coordinates shown are taken directly from result.positions —
    no recomputation occurs in this function.

    Args:
        frame: A pre-captured cropped frame to draw on. Must be a copy
            or the caller must not rely on it after this call.
        result: DetectionResult from the last detection pass.
    """
    display = frame.copy()  # Draw on a copy — never touch the original.
    ...
    # Info bar — coordinates read directly from result.positions, no recomputation.
...
def save_detection_image(frame: np.ndarray, result: DetectionResult) -> None:
    """Saves the detection result image to disk.

    Draws on a copy of the provided frame. Saves as result.jpg.
    If the file already exists, saves as result(1).jpg, result(2).jpg etc.

    Args:
        frame: A pre-captured cropped frame to draw on.
        result: DetectionResult from the last detection pass.
    """
    display = frame.copy()
    for i, ((px, py), (wx, wy), red) in enumerate(
...
def run_homography_setup(camera: Camera, homography: Homography) -> None:
    ...
    def on_mouse(event: int, x: int, y: int, flags: int, param: None) -> None:
        if event == cv2.EVENT_MOUSEMOVE:
            mouse_pos[0] = (x, y - _BAR_HEIGHT)  # Convert to image coordinates for drawing.
        elif event == cv2.EVENT_LBUTTONDOWN and len(clicks) < 4:
            clicks.append((x, y - _BAR_HEIGHT))  # Convert to image coordinates for drawing.
            print(
